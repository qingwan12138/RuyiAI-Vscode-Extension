...
**实施进度**：缺口 1 与 2 已由 T1 修复（拒绝变为可区分的工具结局 + 连续 3/累计 20 的拒绝预算 + 简报移到历史之后并改为"照常尝试、读拒绝结果"）。缺口 3 属 T2，**尚未实施**——`plan` 目前仍是"协作模式 + 只读执行级别"的混合体。T3（正交技术边界轴、结构化升级、持久化 allow 规则）未实施。
