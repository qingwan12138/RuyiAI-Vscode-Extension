...
**已完成的验证证据（①联网补强：native 后端，v0.13 收口）**：`test/websearch-native-backend.test.js`（14：请求只声明服务端搜索工具、结果只来自结构化块且**服务商正文被丢弃**、重复 URL 去重、**无搜索块报错而非空结果**、服务商错误码透出、HTTP 失败点名端点且**密钥脱敏**、非 JSON 与取消区分、端点两种写法归一、**后端选择优先级与显式关闭**）。新增 `scripts/probe-native-search.js`（`npm run websearch:probe`）：用真 key 一条命令验证端点是否支持服务端搜索，成功打印真实结果、失败原样打印服务商错误。**真实端点证据**：假 key 打到 `https://api.deepseek.com/anthropic/v1/messages` 得到 **HTTP 401** —— 证明请求形状（URL、`anthropic-version`、`x-api-key`、JSON body）被端点接受、只卡鉴权。**关键设计决定**：服务端搜索**必须**做成 `SearchBackend`（客户端工具）而不是"对话内的服务端工具"，否则它绕过 `PermissionEngine`、Plan 拦不住、审批卡片不出现（ADR-0014 规则 1）。**又一次否决了看似更省事的方案**：把 provider 换成 Anthropic 会让 agent 失去全部工具（`AnthropicProvider` 是 `toolCalling: false`），把产品退化成聊天框。全量 **701 tests / 700 pass / 0 fail / 1 skip**（上一项为 687，+14）。**未在本机验证**：真 key + 真搜索结果 + 真 VS Code 内的审批卡片。

**修复（测试基础设施）：临时目录泄漏**。审计"本地还有没有未提交的东西"时发现：`test/headless.test.js` 与 `test/mcp-stdio-transport.test.js` 用 `mkdtempSync` 建系统临时目录后**完全不清理**，机器上已累积 **327 个**（119 个非空，里面有 `.yisi`/`AGENTS.md`/`written.txt` 等真实文件）。**这类缺陷不会让任何断言失败**，所以一直没人发现——不可失败的缺陷必须由别的东西来抓。**修法（结构性，而不是补一次 rm）**：新增 `test/support/tempDir.js`，创建即注册进进程级集合、`process.on('exit')` **同步**清理（测试抛错或提前退出也照样清），两个泄漏点改用该助手。守卫 `test/temp-dir-hygiene.test.js`（3）：①源码扫描 `test/**`——建临时目录的文件必须引用清理，否则报出文件名；②助手自身语义；③**真实子进程**创建目录后退出，父进程断言该目录**必须已消失**——源码扫描只能证明"写了 rm"，证明不了"rm 真的执行了"。**验证证据**：全量跑一遍（704 tests）前后对比，临时目录 **327 → 327，新增 0**（修复前每跑一次都会新增若干）；旧的 327 个已清理为 0。全量 **704 tests / 703 pass / 0 fail / 1 skip**（上一项为 701，+3）。**未写 ADR**：不改变任何产品/架构决定。
