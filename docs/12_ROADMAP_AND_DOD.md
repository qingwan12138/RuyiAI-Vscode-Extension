...
当前实现状态（2026-09-05，context 防护 + plan/todo + repo index + capability 降级已落地）：新增 **`ContextCompactor`**（application/context）：基于模型 context window，把超出历史预算的最早 turns 截断，保留最近的 turns，并在请求里前置一并 system 说明（"[Earlier conversation omitted: N older message(s)...] 以最新指令为准"）。`ChatService.send` 在 provider 声明了 `maxContextTokens` 时触发（缺省则不压缩，与既有 attachment budget 行为一致）；`estimateTokens` 有界，超大输入不崩溃。DoD "长会话不因简单 context overflow 崩溃" 已由该护栏满足。新增 **`RepoIndexService` + `repo_index` 工具**（readOnly）：单仓库**有界索引**（源码/目录计数、语言分布、顶层条目），跳过 bin/ignored/隐藏目录，尊重根 `.gitignore`（basename），受 file/depth/time 预算 + 取消 + symlink 环保护（docs/16 §15）。新增 **`AgentPlanService` + `plan_todo` 工具**（readOnly，会话级任务清单）。新增 **`ModelCapabilitiesService` + `model_capabilities` 工具**（readOnly）：报告当前选中模型的能力（tool calling/vision/reasoning/streaming/context window）并列出**缺失项（degraded）**，让模型显式降级。覆盖：`test/context-compactor.test.js`、`test/chat-service.test.js`、`test/repo-index-service.test.js`、`test/agent-plan-service.test.js`、`test/model-capabilities-service.test.js`。**待续**：history management 统一管理、cost/context 控制面板、把跨 provider 的 `modelContextWindow` 兜底线程进压缩。
...
当前实现状态（2026-09-05，安全 + 依赖/许可证守卫 + schema 迁移守卫已落地）：新增 **`SecretRedactor`**（application/security）：按精确密钥值替换 + 常见形态统一转为 `[REDACTED]`；已接入 `OpenAICompatibleProvider.requireSuccess`，Provider 错误消息永不回显 API key。新增 **依赖/许可证守卫** `test/dependency-notices.test.js`（每个运行时依赖须登记在 `THIRD_PARTY_NOTICES.md`，拦截未审核 native addon）。新增 **schema/迁移守卫** `test/session-schema.test.js`：`parseSessionDocument` 接受合法 v1 文档并 round-trip、**拒绝未来/未知 schemaVersion**（迁移守卫）、拒绝畸形结构与 active-session 不匹配、空文档合法——保证不兼容的持久化状态绝不静默按 v1 加载。覆盖：`test/secret-redactor.test.js`、`test/openai-compatible-provider.test.js`（错误体脱敏）、`test/dependency-notices.test.js`、`test/session-schema.test.js`。**待续**：性能基线、兼容矩阵、文档补齐、redaction 接入更多日志点。
