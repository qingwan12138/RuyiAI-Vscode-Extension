...
当前实现状态（2026-09-05，context 防护 + plan/todo + repo index 已落地）：新增 **`ContextCompactor`**（application/context）：基于模型 context window，把超出历史预算的最早 turns 截断，保留最近的 turns，并在请求里前置一并 system 说明（"[Earlier conversation omitted: N older message(s)...] 以最新指令为准"）。`ChatService.send` 在 provider 声明了 `maxContextTokens` 时触发（缺省则不压缩，与既有 attachment budget 行为一致）；`estimateTokens` 有界，超大输入不崩溃。DoD "长会话不因简单 context overflow 崩溃" 已由该护栏满足。新增 **`RepoIndexService` + `repo_index` 工具**（readOnly）：对单仓库做**有界索引**（源码/目录计数、语言分布、顶层条目），默认跳过 `.git/node_modules/dist/build/vendor`/隐藏目录与 binary 文件，尊重根 `.gitignore`（basename 模式），并受 **file/depth/time 预算 + 取消 + symlink 环保护**约束，大仓库不撑爆上下文（docs/16 §15）。新增 **`AgentPlanService` + `plan_todo` 工具**（readOnly，会话级任务清单 add/mark/list/clear，仅内存态）。覆盖：`test/context-compactor.test.js`、`test/chat-service.test.js`（压缩实验）、`test/repo-index-service.test.js`（语言计数、跳过 binary/ignored/symlink、maxFiles 截断、abort 提前返回、工具元数据）、`test/agent-plan-service.test.js`。**待续**：history management 统一管理、provider capability **degradation** 显式台阶、cost/context 控制面板、跨 provider 的 `modelContextWindow` 兜底线程进压缩。
