...
当前实现状态（2026-09-05，context 防护 + plan/todo 已落地）：新增 **`ContextCompactor`**（application/context）：基于模型 context window，把超出历史预算的最早 turns 截断，保留最近的 turns，并在请求里前置一并 system 说明（"[Earlier conversation omitted: N older message(s)...] 以最新指令为准"）。`ChatService.send` 在 provider 声明了 `maxContextTokens` 时触发（缺省则不压缩，与既有 attachment budget 行为一致）；`estimateTokens` 有界，超大输入不崩溃。DoD "长会话不因简单 context overflow 崩溃" 已由该护栏满足。新增 **`AgentPlanService` + `plan_todo` 工具**（Mature Agent）：模型在 agent loop 内维护会话级任务清单（add/mark in-progress/done/list/clear），仅内存态、不触工作区/环境故为 readOnly（无需批准）。覆盖：`test/context-compactor.test.js`（预算内不压缩、超预算截断保尾部、空/超大输入不崩溃）、`test/chat-service.test.js`（provider 声明窗口时压缩并前置 system note、消息数有界）、`test/agent-plan-service.test.js`（add/list/update/clear、会话隔离、tool 校验）。**待续**：repo map/index、history management 的统一管理，provider capability **degradation** 的显式台阶（tool-calling/vision 缺失的明确降级与 UI 提示，现为能力 profile 忽略不支持字段 + 明确报错）、cost/context 控制面板，以及把跨 provider 的 `modelContextWindow` 兜底线程进压缩。
