...
当前实现状态（2026-09-05，context 防护 + plan/todo + repo index + capability 降级 + cost/context 控制已落地）：新增 **`ContextCompactor`**（application/context）：基于模型 context window，把超出历史预算的最早 turns 截断，保留最近的 turns，并在请求里前置一并 system 说明（"[Earlier conversation omitted: N older message(s)...] 以最新指令为准"）。`ChatService.send` 在 provider 声明了 `maxContextTokens` 时触发（缺省则不压缩，与既有 attachment budget 行为一致）；`estimateTokens` 有界，超大输入不崩溃。DoD "长会话不因简单 context overflow 崩溃" 已由该护栏满足。**cost/context 控制**：`ChatService` 新增 `historyBudgetRatio`（默认 0.6）——历史只用窗口的一部分，另留回复与后续轮次空间；用户可调低以收紧成本/历史保留。新增 **`RepoIndexService` + `repo_index` 工具**、**`AgentPlanService` + `plan_todo` 工具**、**`ModelCapabilitiesService` + `model_capabilities` 工具**。覆盖：`test/context-compactor.test.js`、`test/chat-service.test.js`（压缩 + 比例控制）、`test/repo-index-service.test.js`、`test/agent-plan-service.test.js`、`test/model-capabilities-service.test.js`。**待续**：history management 统一管理（会话历史折叠/查询 UI），以及 v1.0 交付项（需发布环境）。
