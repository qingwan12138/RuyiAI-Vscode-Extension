...
8c. **Agent 请求必须带两段 system 内容**（详见 docs/06 与 AGENTS.md 的 "Agent system prompt rule"）：**头部**是稳定的"角色 + 工具使用纪律"（`application/agent/agentSystemPrompt.ts`，`AgentToolLoop.run()` unshift 进去，可进缓存前缀），**历史之后、当前用户轮之前**是随模式变化的权限简报（`permissionModePrompt.ts`）。历史事故：agent 路径原先**完全没有 system prompt**，模型只拿到对话 + ~24 个工具定义 + `tool_choice: 'auto'`，被问"请你介绍一下RISC-V吧"这种通识问题时因 `ruyi_check` 描述里含 "RISC-V" 字样而先去查 ruyi、再列目录。**不要**把模式限制写进角色提示（会破坏缓存前缀、混淆两种关注点），也**不要**把角色纪律写进模式简报。
