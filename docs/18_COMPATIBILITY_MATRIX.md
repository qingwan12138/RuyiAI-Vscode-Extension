...
| area | count | notes |
| --- | --- | --- |
| full `node --test test/*.test.js` | **704 tests / 703 pass / 0 fail (1 skip)** | 1 skip = Windows symlink case in the workspace FS suite. |
| Temp-directory hygiene (`test/temp-dir-hygiene.test.js`) | 3 pass | A test that creates a system temp directory must clean it up: a source scan over `test/**` (create without cleanup fails the guard), the shared helper's own semantics, and — the part a scan cannot prove — **a real child process that creates a directory, exits, and leaves nothing behind**. Added after an audit found 327 leftover directories (119 non-empty) leaked by two files. |
