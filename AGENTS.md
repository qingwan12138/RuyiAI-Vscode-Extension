...
## Test temp-directory rule

- 测试要建**系统临时目录**时**必须**走 `test/support/tempDir.js` 的 `tempDirSync()` / `tempDir()`；**禁止**在测试里直接 `fs.mkdtemp*`。该助手创建时注册、在 `process.on('exit')` 里**同步**清理，所以"忘了清理"在结构上不可能发生。
- 理由是一次真实的漏检：`test/headless.test.js` 与 `test/mcp-stdio-transport.test.js` 建了系统临时目录却完全不清理，机器上累积到 **327 个**（119 个非空）才在审计中被发现。**泄漏不会让任何断言失败**——一个不可失败的缺陷只能靠专门的守卫去抓，不能靠运气。
- 守卫 `test/temp-dir-hygiene.test.js` 的两半缺一不可：**源码扫描**（建临时目录的文件必须引用清理，否则报出文件名）+ **真实子进程退出后目录必须消失**。只有源码扫描会接受一个"写了但永远走不到"的 `rm`。
- 修这类问题**先补守卫再修现场**，并给出前后对比证据（本次为"全量跑一遍后新增临时目录 0 个"），否则无法证明真的堵住了。
