...
- Explicit file attachments persist only `{ type, path, workspaceFolderUri, location, uri }` references on the user message (workspace files carry `workspaceFolderUri`; external files carry `location: 'external'` and an absolute `uri`). Raw file content remains transient in the Extension Host and is cleared after the next send.
