...
| --- | --- | --- | --- |
| pdfjs-dist | 3.11.174 | Apache-2.0 | Mozilla PDF.js. Loaded as CommonJS legacy (`require('pdfjs-dist/legacy/build/pdf.js')`) so it works inside the Electron extension host (the 4.x ESM build fails with “PDF parser could not initialize correctly…”), `GlobalWorkerOptions.workerSrc` is set to `pdfjs-dist/legacy/build/pdf.worker.js`, and the worker module is preloaded onto `globalThis.pdfjsWorker` so pdf.js reuses it directly instead of dynamically importing it (which trips “document is not defined”, or a browser-like bare `TypeError`, in the host). Used only for text-layer extraction (`getDocument({ data, isEvalSupported: false, verbosity: 0 })` → `page.getTextContent`); rendering/`canvas`/worker features are never used (`canvas` is not a dependency; DOMMatrix/Path2D polyfill warnings are harmless and expected). |
