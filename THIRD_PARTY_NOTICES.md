...
| --- | --- | --- | --- |
| pdfjs-dist | 4.10.38 | Apache-2.0 | Mozilla PDF.js. Loaded as ESM (`pdfjs-dist/legacy/build/pdf.mjs`) via dynamic `import`. Used only for text-layer extraction (`getDocument({ data, isEvalSupported: false, verbosity: 0 })` → `page.getTextContent`); rendering/`canvas`/worker features are never used. |
