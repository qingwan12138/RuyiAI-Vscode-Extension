...
| --- | --- | --- | --- |
| pdfjs-dist | 3.11.174 | Apache-2.0 | Mozilla PDF.js. Loaded as CommonJS legacy (`require('pdfjs-dist/legacy/build/pdf.js')`) so it works inside the Electron extension host (the 4.x ESM build fails with “PDF parser could not initialize correctly…”). Used only for text-layer extraction (`getDocument({ data, isEvalSupported: false, verbosity: 0 })` → `page.getTextContent`); rendering/`canvas`/worker features are never used (`canvas` is not a dependency; DOMMatrix/Path2D polyfill warnings are harmless and expected). |
